...
<script type="text/javascript">
	$(function(){
        ...
        //controller="Prestamo_socio";
          $( "#q" ).autocomplete({
   minLength: 0,
   source: "index.php?controller=Libro&action=search_autor",
    ...
    select: function( event, ui ) {
			...
			var p=$('#p').val();
                $.ajax({
                    type: "GET",
                    url: "index.php",
                    data: "controller=Libro&action=mostrar_autor&q="+q,
...
<div class='ui-widget-header' style="text-align:center;color:white;">Lista de Autores</div>
<div id="frmtotal">
	<div id="msg" ></div>
	<form id="_form" name="_form" action="" method="GET" style="text-align:center;">
		<input type="hidden" name="controller" value="Libro" />
        <input type="hidden" name="action" value="mostrar_autor" />
	...
	<table  table width="100%" class='table table-hover table-striped table-bordered' style="border:1px solid #666; font-size:14px; margin-bottom:5px;" align="center">
		...
		<tr style="border-bottom:1px solid #666; background:#F0F0F0;">
			...
			<td align="left"><?php  echo utf8_encode($val[2]); ?></td>
                        ...
                        <td align="center"><a href="javascript:devolver_autor('<?php  echo $val[0]; ?>','<?php  echo utf8_encode($val[1]); ?>','<?php  echo utf8_encode($val[2]); ?>');" title="add editorial"><i class="icon-arrow-down btn btn-small"></i></a></td>
		</tr>
			<?php }?>
	</table>
	<?php echo $pag;?>
